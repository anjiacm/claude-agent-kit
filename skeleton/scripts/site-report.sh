#!/bin/bash
# 综合站点报告 — 性能 + SEO + GEO + 流量，生成飞书 Wiki 文档
...
PERF_REPORT="" SEO_REPORT="" GEO_REPORT="" TRAFFIC_REPORT=""
...
# ── 4. 流量分析 ──
echo "▶ [4/4] 流量分析..."
TRAFFIC_REPORT=$(bash "$SCRIPT_DIR/traffic-report.sh" 2>&1 | tail -1)
echo "  → $TRAFFIC_REPORT"
echo ""
...
if [ -n "$TENANT" ]; then
  DOC_URL=$(bash "$SCRIPT_DIR/feishu-site-report.sh" "$PERF_REPORT" "$SEO_REPORT" "$GEO_REPORT" "$TRAFFIC_REPORT" "$TENANT" 2>/dev/null)
  echo ""
  echo "══════════════════════════════════════"
  echo "📄 报告文档: $DOC_URL"
  echo "══════════════════════════════════════"
